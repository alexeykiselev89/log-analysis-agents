...
class PromptBuilder:
    ...
    def build_prompt(data: Union[dict, Iterable[ClassifiedError]]) -> str:
        ...
        prompt_intro = (
            ...
            "описание ошибки, несколько примеров исходных сообщений из лога (вместе со "
            "stacktrace), имя класса, частоту появления и предварительную оценку критичности.\n"
            ...
            "2. Используя примеры сообщений (в том числе stacktrace), кратко описать возможную первопричину: какие компоненты, конфигурации, таблицы, методы или внешние сервисы могут вызывать эту ошибку.\n"
            "3. Сформулировать подробную и конкретную рекомендацию по устранению проблемы. Избегай общих фраз; перечисли как минимум пять последовательных шагов, которые должен выполнить инженер: какие классы, методы, таблицы, поля, индексы или параметры конфигурации нужно проверить, изменить или создать; какие команды или SQL‑запросы выполнить; какие логи, метрики или конфигурации дополнительно изучить.\n"
            "4. При возможности приведи пример кода, SQL‑запроса или конфигурации, который поможет исправить ошибку.\n"
            "5. Опиши, как проверить эффективность решения: какие тесты выполнить, какие логи или отчёты проанализировать.\n"
            "6. Если для точного диагноза недостаточно информации, укажи, какую дополнительную информацию нужно собрать (например, полный stacktrace, SQL‑запрос, конфигурационный файл или дамп таблицы).\n\n"
        ...
        for err in top_errors:
            # original_message может содержать несколько примеров, разделённых
            # последовательностью "---". Показываем это явно в промпте.
